					...
					<Card className="grid h-fit w-full grid-cols-1 gap-3 p-card-surround">
						...
						{projectEnvironments?.length ? (
							<section className="flex flex-wrap items-start justify-start gap-1">
								<h3 className="block w-full font-bold text-foreground-muted">{t.project.environments}</h3>
					...
					<Card className="grid grid-cols-1 gap-3 p-card-surround">
						...
						{projectData?.discordInviteUrl ? (
							<ExternalLink
								...
								icon={<DiscordIcon aria-hidden className="h-btn-icon w-btn-icon fill-current dark:fill-current" />}
							/>
						) : null}
					</Card>
				) : null}
