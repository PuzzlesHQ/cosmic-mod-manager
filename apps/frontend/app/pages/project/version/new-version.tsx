					...
					<div className="flex max-w-full flex-col gap-panel-cards overflow-auto">
						...
						<ContentCardTemplate title={t.version.files} className="grid gap-form-elements">
											...
											<input
												...
												onChange={async (e) => {
													...
													if (additionalFiles.find((f) => f.name === file.name)) {
														return toast.error(`File "${file.name}" already exists in the additional files`);
													}
							...
							<FormField
								...
								render={({ field }) => (
									<SelectAdditionalProjectFiles fieldName={field.name} value={field.value} onChange={field.onChange} />
								)}
							/>
						</ContentCardTemplate>
					</div>
