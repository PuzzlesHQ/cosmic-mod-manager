...
function Card({ ref, className, useSectionTag, ...props }: CardProps) {
	...
	return (
		<Tag ref={ref} className={cn("rounded-lg bg-card-background text-foreground shadow-sm", className)} {...props} />
	);
}
...
function CardFooter({ ref, className, ...props }: React.ComponentProps<"div">) {
	return (
		<div ref={ref} className={cn("flex items-center justify-end gap-3 p-card-surround pt-0", className)} {...props} />
	);
}
