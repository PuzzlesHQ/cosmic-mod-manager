...
Set `Config.BACKEND_URL_PUBLIC` and `Config.BACKEND_URL_LOCAL` to `https://api.crmods.org` and `Config.proxy = true` in [`app/utils/config.ts`](/apps/frontend/app/utils/config.ts#L25); and now all the api requests will be made to the prod backend.
