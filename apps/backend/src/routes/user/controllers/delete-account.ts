...
export async function confirmUserAccountDeletion(token: string) {
    ...
    if (!confirmationEmail?.id || !isExpired(confirmationEmail.dateCreated, DELETE_USER_ACCOUNT_EMAIL_VALIDITY_ms)) {
    ...
    await deleteAllUserCollections(user.id, user as Pick<SessionUserData, "id" | "role">);
