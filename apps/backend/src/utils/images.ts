...
import sharp, { type FitEnum, type KernelEnum, type ResizeOptions } from "sharp";

type ObjVal<T> = T[keyof T];

interface ResizeProps {
    width?: number;
    height?: number;
    fit?: ObjVal<FitEnum>;
    kernel?: ObjVal<KernelEnum>;
...
export async function resizeImageToWebp(
    ...
    if ((metadata.width || 0) > 256 && (metadata.height || 0) > 256) defaultKernel = sharp.kernel.lanczos3;
